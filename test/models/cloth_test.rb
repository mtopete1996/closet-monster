...
class ClothTest < ActiveSupport::TestCase
  ...
  test 'CREATE cloth w nil user' do
    ...
    assert cloth.invalid?, 'Cloth should be invalid'
    assert_not cloth.persisted?, 'Cloth should not have persisted'
    assert_includes cloth.errors.full_messages, 'User must exist', 'There should be an error of user presence'
  end
  ...
  test 'CREATE cloth w last_worn is future' do
    ...
    assert_includes errors, 'Last time worn can not be in the future', 'There should be an error wrong date'
  end
end
